...
wasmBinaryFile = "SpriteEditorWeb.wasm";
...
var PThread = {
 ...
 allocateUnusedWorker() {
  var worker;
  var pthreadMainJs = locateFile("SpriteEditorWeb.worker.js");
...
function ___syscall_symlink(target, linkpath) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(19, 0, 1, target, linkpath);
 target >>>= 0;
 linkpath >>>= 0;
 try {
...
function ___syscall_truncate64(path, length) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(20, 0, 1, path, length);
 path >>>= 0;
 length = bigintToI53Checked(length);
 try {
  if (isNaN(length)) return 61;
...
function ___syscall_utimensat(dirfd, path, times, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(22, 0, 1, dirfd, path, times, flags);
 path >>>= 0;
 times >>>= 0;
 try {
  path = SYSCALLS.getStr(path);
...
var ___start_em_js = Module["___start_em_js"] = 8772484;

var ___stop_em_js = Module["___stop_em_js"] = 8773574;
...
if (typeof exports === 'object' && typeof module === 'object')
  module.exports = SpriteEditorWeb_entry;
else if (typeof define === 'function' && define['amd'])
  define([], () => SpriteEditorWeb_entry);
